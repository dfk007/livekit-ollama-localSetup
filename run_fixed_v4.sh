...
async def main():
    ...
    try:
        ...
        # Try different approaches to connect to LiveKit
        try:
            # Approach 1: Pass parameters directly to start()
            await session.start(
                agent,
                url=livekit_url,
                api_key=api_key,
            ...
            print(f"[Agent] First approach failed: {e1}")
            try:
                # Approach 2: Use livekit_url parameter
                await session.start(
                    agent,
                ...
                print(f"[Agent] Second approach failed: {e2}")
                # Approach 3: Use signal_url parameter
                await session.start(
                    agent,
